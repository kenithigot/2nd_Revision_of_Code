...
//Launching of Application
	...
	private void initialize() {
		...
		JLabel lblMR_KEY_CHECKER = new JLabel(Messages.getString("Key_Checker.0")); //$NON-NLS-1$
		...
		lblMR_KEY_CHECKER.setFont(new Font( Font.BOLD, 20)); //$NON-NLS-1$
		...
		JLabel lblNAME = new JLabel(Messages.getString("Key_Checker.2")); //$NON-NLS-1$
		panel_NAME.add(lblNAME);
		lblNAME.setFont(new Font(Messages.getString("Key_Checker.3"), Font.BOLD, 16)); //$NON-NLS-1$
		...
		JLabel lblKEY_ID = new JLabel(Messages.getString("Key_Checker.4")); //$NON-NLS-1$
		panel_KEY_ID.add(lblKEY_ID);
		lblKEY_ID.setVerticalAlignment(SwingConstants.TOP);
		lblKEY_ID.setFont(new Font(Messages.getString("Key_Checker.5"), Font.BOLD, 16)); //$NON-NLS-1$
		...
		JLabel lblNewLabel = new JLabel(Messages.getString("Key_Checker.6")); //$NON-NLS-1$
		panel_DATE_RETURNED.add(lblNewLabel);
		lblNewLabel.setFont(new Font(Messages.getString("Key_Checker.7"), Font.BOLD, 16)); //$NON-NLS-1$
		...
		JLabel lblDATE_RETURNED = new JLabel(Messages.getString("Key_Checker.8")); //$NON-NLS-1$
		panel_DATE_RETURN.add(lblDATE_RETURNED);
		lblDATE_RETURNED.setFont(new Font(Messages.getString("Key_Checker.9"), Font.BOLD, 16)); //$NON-NLS-1$
		
		//Logout Button
		JButton btnLOGOUT = new JButton(Messages.getString("Key_Checker.10")); //$NON-NLS-1$
		...
		{private JFrame frame;
			public void actionPerformed(ActionEvent e) {
				frame = new JFrame(Messages.getString("Key_Checker.11")); //$NON-NLS-1$
				if(JOptionPane.showConfirmDialog(frame,  Messages.getString("Key_Checker.12"), Messages.getString("Key_Checker.13"),JOptionPane.YES_NO_OPTION)==JOptionPane.YES_OPTION) { //$NON-NLS-1$ //$NON-NLS-2$
					System.exit(0);
				}
			}
		});
		btnLOGOUT.setFont(new Font(Messages.getString("Key_Checker.14"), Font.BOLD, 13)); //$NON-NLS-1$
		...
		JButton btnADD = new JButton(Messages.getString("Key_Checker.15")); //$NON-NLS-1$
		...
		btnADD.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {	
				if (txtNAME_SLOT.getText().equals(Messages.getString("Key_Checker.16")) || txtKEY_ID_SLOT.getText().equals(Messages.getString("Key_Checker.17")) || txtDATE_BARROWED_SLOT.getText().equals(Messages.getString("Key_Checker.18")) || txtDATE_RETURNED_SLOT.getText().equals(Messages.getString("Key_Checker.19"))) //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
				 {
					JOptionPane.showMessageDialog(null, Messages.getString("Key_Checker.20"));	 //$NON-NLS-1$
					}
				else {
				...
				model.addRow(new Object [] { txtNAME_SLOT.getText(),txtKEY_ID_SLOT.getText(), txtDATE_BARROWED_SLOT.getText(), txtDATE_RETURNED_SLOT.getText()});
					JOptionPane.showMessageDialog(null, Messages.getString("Key_Checker.21"));	 //$NON-NLS-1$
				}
			}	
		});
		btnADD.setForeground(Color.BLACK);
		btnADD.setFont(new Font(Messages.getString("Key_Checker.22"), Font.BOLD, 7)); //$NON-NLS-1$
		frame.getContentPane().add(btnADD);
		
		//Clear Information Button
		JButton btnCLEAR = new JButton(Messages.getString("Key_Checker.23")); //$NON-NLS-1$
		...
		btnCLEAR.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				txtNAME_SLOT.setText(Messages.getString("Key_Checker.24")); //$NON-NLS-1$
				txtKEY_ID_SLOT.setText(Messages.getString("Key_Checker.25")); //$NON-NLS-1$
				txtDATE_BARROWED_SLOT.setText(Messages.getString("Key_Checker.26")); //$NON-NLS-1$
				txtDATE_RETURNED_SLOT.setText(null);	
			}
		});
		btnCLEAR.setForeground(Color.BLACK);
		btnCLEAR.setFont(new Font(Messages.getString("Key_Checker.27"), Font.BOLD, 7)); //$NON-NLS-1$
		frame.getContentPane().add(btnCLEAR);
		
		//Button delete information
		JButton btnDELETE = new JButton(Messages.getString("Key_Checker.28")); //$NON-NLS-1$
			...
			public void actionPerformed(ActionEvent arg0) {	
				...
				if (table.getSelectedRowCount() ==1 ) {
					...
					if (table.getRowCount() == 0 ) {
						JOptionPane.showMessageDialog(null, Messages.getString("Key_Checker.29")); //$NON-NLS-1$
					}
					else {
						JOptionPane.showMessageDialog(null, Messages.getString("Key_Checker.30")); //$NON-NLS-1$
					}
				}
			}
		});
		btnDELETE.setFont(new Font(Messages.getString("Key_Checker.31"), Font.BOLD, 7)); //$NON-NLS-1$
		btnDELETE.setForeground(Color.BLACK);
		frame.getContentPane().add(btnDELETE);
		
		//Update information Button
		JButton btnUPDATE = new JButton(Messages.getString("Key_Checker.32")); //$NON-NLS-1$
		...
		btnUPDATE.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {	
				...
				if (table.getSelectedRowCount() == 1) {
					...
					JOptionPane.showMessageDialog(null, Messages.getString("Key_Checker.33"));	 //$NON-NLS-1$
				}
				else {
					if (table.getRowCount() == 0) {
					JOptionPane.showMessageDialog(null, Messages.getString("Key_Checker.34"));	 //$NON-NLS-1$
					}
					else {
						JOptionPane.showMessageDialog(null, Messages.getString("Key_Checker.35"));		 //$NON-NLS-1$
					}
				}
			}
		});
		...
		btnUPDATE.setFont(new Font(Messages.getString("Key_Checker.36"), Font.BOLD, 7)); //$NON-NLS-1$
		frame.getContentPane().add(btnUPDATE);
		
		JScrollPane scrollPane_1 = new JScrollPane();
		scrollPane_1.setBackground(SystemColor.inactiveCaption);
		scrollPane_1.setFont(new Font(Messages.getString("Key_Checker.37"), Font.BOLD, 12)); //$NON-NLS-1$
		...
		table.setFont(new Font(Messages.getString("Key_Checker.38"), Font.PLAIN, 12)); //$NON-NLS-1$
		table.setAlignmentY(Component.TOP_ALIGNMENT);
		table.setModel(new DefaultTableModel( new Object[][] {}, new String[] { Messages.getString("Key_Checker.39"), Messages.getString("Key_Checker.40"), Messages.getString("Key_Checker.41"), Messages.getString("Key_Checker.42") } //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
		));
		...
		JLabel lblKEY_LOG_SYSTEM = new JLabel(Messages.getString("Key_Checker.43")); //$NON-NLS-1$
		...
		lblKEY_LOG_SYSTEM.setToolTipText(Messages.getString("Key_Checker.44")); //$NON-NLS-1$
		lblKEY_LOG_SYSTEM.setFont(new Font(Messages.getString("Key_Checker.45"), Font.BOLD, 20)); //$NON-NLS-1$
